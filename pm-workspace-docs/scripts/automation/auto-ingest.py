...
This script watches the inbox folder for new files and:
...
4. Optionally queues items for signal-router
5. Optionally sends Slack notifications for new signals

Usage:
    python auto-ingest.py                    # Process all inbox files once
    python auto-ingest.py --watch            # Watch continuously (every 60s)
    python auto-ingest.py --route            # Queue for signal-router
...
ROUTER_QUEUE = INBOX_DIR / "router-queue.json"
...
def process_inbox_file(inbox_path: Path, signal_type: str, dry_run: bool = False, route: bool = False) -> Optional[dict]:
...
"""
    ...
    if route:
        queue = []
        if ROUTER_QUEUE.exists():
            try:
                queue = json.loads(ROUTER_QUEUE.read_text())
            except Exception:
                queue = []
        queue.append({
            "signal_path": str(dest_path.relative_to(WORKSPACE_ROOT)),
            "signal_id": signal_entry["id"]
        })
        ROUTER_QUEUE.parent.mkdir(parents=True, exist_ok=True)
        ROUTER_QUEUE.write_text(json.dumps(queue, indent=2))
...
def process_all_inboxes(dry_run: bool = False, route: bool = False) -> list:
    ...
    for inbox_type, signals_type in INBOX_TO_SIGNALS.items():
        ...
        if files:
            ...
            for file_path in files:
                result = process_inbox_file(file_path, inbox_type, dry_run, route)
                if result:
...
def watch_mode(interval: int = 60, slack: bool = False, route: bool = False):
    ...
    while True:
        try:
            processed = process_all_inboxes(route=route)
...
def main():
    ...
    parser.add_argument("--route", action="store_true", help="Queue for signal-router")
    ...
    if args.watch:
        watch_mode(args.interval, args.slack, args.route)
    else:
        processed = process_all_inboxes(args.dry_run, args.route)
