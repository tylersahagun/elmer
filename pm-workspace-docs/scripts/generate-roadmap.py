...
SIGNALS_INDEX_PATH = SIGNALS_DIR / "_index.json"
...
def load_context_updates(existing_roadmap_path: Path) -> List[Dict]:
    """Load approved roadmap context updates from signals and preserve existing notes."""
    updates_by_key: Dict[str, Dict] = {}

    # Preserve existing context updates (to keep added_at stable)
    if existing_roadmap_path.exists():
        try:
            existing = json.loads(existing_roadmap_path.read_text())
            for update in existing.get("context_updates", []):
                key = update.get("candidate_id") or f"{update.get('note')}|{update.get('source_signal')}"
                if key and key not in updates_by_key:
                    updates_by_key[key] = update
        except json.JSONDecodeError:
            print(f"  Warning: Invalid JSON in {existing_roadmap_path}")

    # Merge in approved context candidates targeting roadmap.json
    if SIGNALS_INDEX_PATH.exists():
        try:
            index = json.loads(SIGNALS_INDEX_PATH.read_text())
            for signal in index.get("signals", []):
                for candidate in signal.get("context_candidates", []):
                    if candidate.get("status") != "approved":
                        continue
                    target_file = candidate.get("target_file", "")
                    if not target_file.endswith("roadmap/roadmap.json"):
                        continue

                    candidate_id = candidate.get("id")
                    key = candidate_id or f"{candidate.get('content')}|{candidate.get('source_signal')}"
                    if not key:
                        continue

                    if key in updates_by_key:
                        updates_by_key[key]["note"] = candidate.get("content", updates_by_key[key].get("note"))
                        updates_by_key[key]["source_signal"] = candidate.get("source_signal", updates_by_key[key].get("source_signal"))
                        updates_by_key[key]["candidate_id"] = candidate_id or updates_by_key[key].get("candidate_id")
                    else:
                        updates_by_key[key] = {
                            "note": candidate.get("content", ""),
                            "source_signal": candidate.get("source_signal"),
                            "candidate_id": candidate_id,
                            "added_at": datetime.now().isoformat()
                        }
        except json.JSONDecodeError:
            print(f"  Warning: Invalid JSON in {SIGNALS_INDEX_PATH}")

    return list(updates_by_key.values())
...
def main():
    ...
    context_updates = load_context_updates(roadmap_json_path)
    if context_updates:
        roadmap["context_updates"] = context_updates
