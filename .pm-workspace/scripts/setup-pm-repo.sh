...
mkdir -p .elmer/{company-context,initiatives,research/user-interviews,meeting-notes,agents-docs,knowledge-base-sync,scripts}
mkdir -p .cursor/{commands,rules}
mkdir -p prototypes/src/components

echo ""
echo "📋 Copying PM workspace files..."
if [ -d "$ELEPHANT_AI_PATH/.elmer" ]; then
    cp -r "$ELEPHANT_AI_PATH/.elmer/"* .elmer/ 2>/dev/null || true
    echo "   ✓ Copied .elmer/"
fi
...
if [ -d "$ELEPHANT_AI_PATH/.elmer/scripts/cursor-rules-for-pm-repo" ]; then
    cp -r "$ELEPHANT_AI_PATH/.elmer/scripts/cursor-rules-for-pm-repo/"* .cursor/rules/ 2>/dev/null || true
    echo "   ✓ Copied PM-specific Cursor rules"
fi

# Copy Slack commands reference
if [ -f "$ELEPHANT_AI_PATH/.elmer/SLACK-COMMANDS.md" ]; then
    cp "$ELEPHANT_AI_PATH/.elmer/SLACK-COMMANDS.md" ./SLACK-COMMANDS.md
...
├── .elmer/            # PM work (PRDs, research, notes)
...
echo "   rm -rf $ELEPHANT_AI_PATH/.elmer"
...
echo "   cp $ELEPHANT_AI_PATH/.elmer/scripts/github-actions/sync-submodule.yml \\"
