...
Read the transcript at .elmer/knowledge-base-sync/{file}.md and:
1. Extract key decisions and action items
2. Identify user problems with verbatim quotes
3. Save analysis to .elmer/initiatives/{initiative_name}/research.md
```

```
@Cursor Please run the PM workflow for the {initiative_name} initiative.
Read the research at .elmer/initiatives/{initiative_name}/research.md and:
...
Read the PRD at .elmer/initiatives/{initiative_name}/prd.md and:
...
```bash
bash .elmer/scripts/setup-pm-repo.sh
```

This will:
1. Create a new directory at `~/Source/elmer`
