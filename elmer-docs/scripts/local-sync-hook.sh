#!/bin/bash
# Local sync hook for elmer
# 
# Option A: Run manually when you remember
#   bash ~/Source/elmer/scripts/sync.sh
#
# Option B: Add as a git hook in elephant-ai (runs on every pull)
#   ln -sf ~/Source/elmer/scripts/sync.sh ~/Source/elephant-ai/.git/hooks/post-merge
#
# Option C: Add as a cron job (runs daily)
#   crontab -e
#   0 8 * * * /Users/tylersahagun/Source/elmer/scripts/sync.sh >> /tmp/pm-sync.log 2>&1

set -e

PM_WORKSPACE="$HOME/Source/elmer"
...
echo -e "${YELLOW}🔄 Syncing elmer submodule...${NC}"
