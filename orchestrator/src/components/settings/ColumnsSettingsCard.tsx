...
const jobTypeOptions: Array<{
  ...
  {
    value: "analyze_transcript",
    label: "Analyze Transcript",
    description: "Extract insights from user research",
  },
  {
    value: "process_signal",
    label: "Process Signal",
    description: "Ingest and normalize incoming signals",
  },
  {
    value: "synthesize_signals",
    label: "Synthesize Signals",
    description: "Cluster and summarize related signals",
  },
  {
    value: "generate_prd",
    label: "Generate PRD",
  ...
  {
    value: "generate_design_brief",
    label: "Generate Design Brief",
    description: "Create design specifications",
  },
  {
    value: "generate_engineering_spec",
    label: "Generate Engineering Spec",
    description: "Create technical specifications",
  },
  {
    value: "generate_gtm_brief",
    label: "Generate GTM Brief",
    description: "Create go-to-market plan",
  },
  {
    value: "run_jury_evaluation",
    label: "Run Jury Evaluation",
    description: "Evaluate with synthetic personas",
  },
  {
    value: "build_prototype",
    label: "Build Prototype",
    description: "Generate prototype components",
  },
  {
    value: "iterate_prototype",
    label: "Iterate Prototype",
    description: "Improve existing prototype",
  },
  {
    value: "generate_tickets",
    label: "Generate Tickets",
    description: "Create engineering tickets",
  },
  {
    value: "validate_tickets",
    label: "Validate Tickets",
    description: "Review and validate tickets",
  },
  {
    value: "execute_agent_definition",
    label: "Execute Agent Definition",
    description: "Run imported skill/command/subagent",
  },
  {
    value: "deploy_chromatic",
    label: "Deploy to Chromatic",
    description: "Deploy Storybook to Chromatic",
  },
  {
    value: "create_feature_branch",
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Configure your pipeline columns. Click a column to expand and edit its
          settings. Toggle visibility instantly. Other changes require clicking
          Save.
        </CardDescription>
      </CardHeader>
          ...
          {columns
            ...
            .map((column) => {
              const colorKey = String(
                getColumnEdit(column.id, "color", column.color || "slate"),
              );
              ...
              const displayNameValue = String(
                getColumnEdit(column.id, "displayName", column.displayName),
              );
                  ...
                  <div
                    ...
                    onKeyDown={(event) => {
                      if (event.key === "Enter" || event.key === " ") {
                        ...
                        toggleColumnExpanded(column.id);
                      }
                    }}
                    className="w-full p-3 flex items-center justify-between gap-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                  >
                      ...
                      >
                        ...
                        <Switch
                          ...
                          onCheckedChange={(checked) =>
                            persistColumnUpdates(column, { enabled: checked })
                          }
                    ...
                    <div className="px-3 pb-4 space-y-4 border-t border-border">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                          ...
                          <div className="space-y-2">
                            ...
                            <Select
                              value={colorKey}
                              onValueChange={(value) =>
                                updateColumnEdit(column.id, "color", value)
                              }
                            >
                              <SelectTrigger>
                                <SelectValue />
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent className="w-64 max-h-72 overflow-y-auto">
                                ...
                                {jobTypeOptions.map((job) => {
                                  ...
                                  const isSelected = selectedJobs.includes(
                                    job.value,
                                  );
                                  return (
                                    <DropdownMenuCheckboxItem
                            ...
                            <p className="text-xs text-muted-foreground">
                              Jobs that run automatically when a project enters
                              this stage
                            </p>
                          </div>
                          <div className="space-y-2">
                            ...
                            <p className="text-xs text-muted-foreground">
                              Documents that must exist before moving to this
                              stage
                            </p>
                          </div>
                        </div>
                        <div className="mt-4 space-y-2">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Agent definitions run automatically when a project
                            enters this stage.
                          </p>
                        </div>
                      </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                              {contextPathsValue.map((path, idx) => (
                                <div
                                  key={`${column.id}-context-${idx}`}
                                  className="flex items-center gap-2"
                                >
                                  <Input
                                    value={path}
                                    onChange={(e) =>
                                      updateColumnContextPath(
                                        column,
                                        idx,
                                        e.target.value,
                                      )
                                  ...
                                  <Button
                                    ...
                                    disabled={
                                      idx === contextPathsValue.length - 1
                                    }
                                    className="h-9 w-9"
                                  >
                                  ...
                                  <Button
                                    ...
                                    variant="ghost"
                                    onClick={() =>
                                      removeColumnContextPath(column, idx)
                                    }
                                    className="h-9 w-9"
                                  >
                                    <Trash2 className="w-4 h-4" />
                    ...
                    <SelectItem key={key} value={key}>
                      <div className="flex items-center gap-2">
                        <div
                          className={cn(
                            "w-3 h-3 rounded-full",
                            stageColors[key]?.bg || "bg-slate-400",
                          )}
                        />
                        <span className="capitalize">{key}</span>
                      </div>
