...
<task type="auto">
  <name>Task 2: Verify integration with existing signals infrastructure</name>
  <files>None (verification only)</files>
  <action>
Verify the upload endpoint integrates correctly with existing infrastructure:

1. Check that SignalSource union type in schema.ts includes "upload":
   - Look for `export type SignalSource = "paste" | "interview" | "email" | "webhook" | "upload" | ...`

2. Check that createSignal in queries.ts accepts source="upload" and sourceMetadata:
   - Look for createSignal function signature and verify it handles sourceMetadata JSONB

3. Check that activityLogs table exists and accepts the metadata structure:
   - Look for activityLogs in schema.ts

If any of these are missing, the upload endpoint will fail at runtime. Report findings.

Expected: All three checks pass because Phase 11 established the signals schema with these fields.
  </action>
  <verify>
Grep for SignalSource type to confirm "upload" is included.
Grep for createSignal function to confirm sourceMetadata parameter.
Grep for activityLogs to confirm table exists.
  </verify>
  <done>Upload endpoint verified to integrate with existing signals schema and queries</done>
</task>
