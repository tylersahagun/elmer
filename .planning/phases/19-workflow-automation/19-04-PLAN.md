...
Add this import after the existing import block (after `import { classifySignal } from "@/lib/classification";`):

```typescript
import { checkSignalAutomationForNewSignal } from "@/lib/automation/signal-automation";
```

**Step 2: Add automation check after classification block**

In the `processSignalExtraction` function, locate the classification try-catch block by searching for the pattern:
...
Immediately AFTER this closing brace of the `if (embeddingVector && embeddingVector.length === 1536)` block (but still INSIDE the outer try block), add:
...
- `getSignal` is already imported on line 1: `import { getSignal, updateSignalProcessing } from "@/lib/db/queries";`
- The classification block is inside the outer try block that starts around line 51
- The catch block for classification ends around line 89-90
- The new code goes after line 90 (the closing brace of the classification if-block)
  </action>
