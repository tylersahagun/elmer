...
must_haves:
  ...
  artifacts:
    ...
    - path: "orchestrator/src/lib/notifications/index.ts"
      provides: "Re-export barrel for cleaner imports"
      exports: ["shouldSendNotification", "createThresholdAwareNotification", "notifyClusterDiscovered"]
  key_links:
...
<task type="auto">
  ...
  <action>
Create directory `orchestrator/src/lib/notifications/` if it doesn't exist, then create `orchestrator/src/lib/notifications/index.ts` with the following complete content:
...
- Before: `import { notifyClusterDiscovered } from "@/lib/notifications/threshold-filter";`
- After: `import { notifyClusterDiscovered } from "@/lib/notifications";`
  </action>
  <verify>TypeScript compiles: `cd orchestrator && npx tsc --noEmit`</verify>
  <done>Notifications module index created at orchestrator/src/lib/notifications/index.ts with all exports from threshold-filter</done>
</task>
